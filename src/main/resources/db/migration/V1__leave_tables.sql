...
create table leave_balances(
                               id serial primary key ,
                               user_id int not null references authentication.users(id) on delete cascade ,
...
create type leave_states as enum (
    'APPROVED','PENDING',
    'REJECTED'
);
...
create table leave_requests(
                               ...
                               leave_status leave_states not null default 'PENDING' ,
                               approver_id int references authentication.users(id) ,
...
CREATE INDEX IF NOT EXISTS idx_leave_balances_type_user
    ON leave_balances(leave_type_id, user_id);
...
CREATE INDEX IF NOT EXISTS idx_leave_requests_status
    ON leave_requests(leave_status);
...
CREATE INDEX IF NOT EXISTS idx_leave_requests_user_status
    ON leave_requests(user_id, leave_status);
...
CREATE INDEX IF NOT EXISTS idx_leave_requests_type_status
    ON leave_requests(leave_type_id, leave_status);
...
CREATE INDEX IF NOT EXISTS idx_leave_requests_approver_status
    ON leave_requests(approver_id, leave_status);


-- =======================
-- Initial data
-- =======================
insert into leave_type(code, description) values
                                              ('SL','Sick leave') ,
                                              ('CL','Casual leave') ,
                                              ('EL','Earned leave') ,
                                              ('CML','Camp-Off leave') ,
